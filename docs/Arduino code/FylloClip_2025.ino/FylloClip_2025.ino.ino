...
void setup() {
  ...
  get_parameters();  //retrieve settings for sleep period, SF, frame counter
...
void saymyname() {
  Serial.println(F("FylloClip_2025.ino"));
}
...
void get_parameters() {
  SF = EEPROM.read(2);      //spreading factor
  if (SF < 7 || SF > 12) {  //SF must be between 07 and 12
    SF = 7;
    implement_SF();
  }

  set_value = EEPROM.read(6);                                                                                           //sleep period
  if (set_value != 5 && set_value != 10 && set_value != 15 && set_value != 20 && set_value != 30 && set_value != 60) {  //sleep period must be either 05,10, 15, 20, 30 or 60, if not set 10 as default
    set_value = 10;
    EEPROM.write(6, set_value);  //save sleep period in EEPROM
  }

  frame_counter = EEPROM.read(4) * 256 + EEPROM.read(5);  //frame counter
  if (frame_counter == 255) {                             //on a new ATMega chip the initial value is generally 255, in this case set the counter to 0
    frame_counter = 0;
  }
}
