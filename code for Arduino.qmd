...
The ABP mode of activating the device requires 3 unique identification codes to be inserted into the sketch before uploading it to the device. These are the **device address** (DevAddr), the **application session key** (AppSkey) and the **network session key** (NwkSkey). You will obtain these codes when creating a new end device on the TTS console, after you will have created your own personal account there.
