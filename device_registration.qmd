...
After having created the application we should specify some settings necessary for the later data handling. One thing to do is the insert a payload formatter, i.e. a code which will transform the incoming data into form suitable for our needs.

To achieve this, select Payload formatters –\> Uplink and then *Custom Javascript formatter* as formatter type. In the field *Formatter code* paste the follwing code:
